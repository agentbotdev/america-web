"use client";

import { motion, useReducedMotion } from "motion/react";
import { ArrowRight } from "lucide-react";
import { waLink, mensajeGeneral } from "@/lib/whatsapp";
import { AGENCIA } from "@/data/agencia";
import { mensajeWhatsapp, type Respuestas } from "./flujo";
import type { Propiedad } from "@/types";

/* ------------------------------------------------------------------ */
/*  Handoff final a WhatsApp con TODA la data capturada. Si el visitante */
/*  no llegó a definir criterios (ej. eligió "hablar con una persona"), */
/*  cae al mensaje general.                                            */
/* ------------------------------------------------------------------ */

export function WaCierre({
  respuestas,
  interesados,
  onReiniciar,
}: {
  respuestas: Respuestas;
  interesados: Propiedad[];
  onReiniciar: () => void;
}) {
  const reduce = useReducedMotion();
  const hayCriterios =
    interesados.length > 0 || Object.keys(respuestas).length > 0;
  const mensaje = hayCriterios
    ? mensajeWhatsapp(respuestas, interesados)
    : mensajeGeneral(AGENCIA);
  const href = waLink(AGENCIA.whatsapp, mensaje);

  return (
    <motion.div
      initial={{ opacity: 0, y: 14 }}
      animate={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.4 }}
      className="mt-1"
      style={{ perspective: 900 }}
    >
      <motion.p
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        transition={{ delay: 0.15 }}
        className="mb-2.5 text-center text-sm font-semibold text-white"
      >
        🎉 ¡Listo! Te conecto con un asesor real ahora.
      </motion.p>

      <motion.a
        href={href}
        target="_blank"
        rel="noopener noreferrer"
        initial={{ opacity: 0, rotateX: -55, scale: 0.82 }}
        animate={{ opacity: 1, rotateX: 0, scale: 1 }}
        transition={{ type: "spring", stiffness: 240, damping: 16, delay: 0.1 }}
        whileHover={reduce ? undefined : { scale: 1.04, y: -2 }}
        whileTap={{ scale: 0.97 }}
        className="group relative flex items-center justify-center gap-2 overflow-hidden rounded-2xl bg-gradient-to-br from-emerald-500 to-whatsapp px-4 py-4 text-sm font-bold text-white shadow-xl shadow-emerald-900/50"
        style={{ transformStyle: "preserve-3d" }}
      >
        {!reduce && (
          <motion.span
            aria-hidden
            className="pointer-events-none absolute inset-0 rounded-2xl"
            animate={{ boxShadow: ["0 0 0 0 rgba(16,185,129,0.55)", "0 0 0 12px rgba(16,185,129,0)"] }}
            transition={{ duration: 1.9, repeat: Infinity }}
          />
        )}
        {!reduce && (
          <motion.span
            aria-hidden
            className="pointer-events-none absolute inset-0 -translate-x-full bg-white/25 [mask-image:linear-gradient(90deg,transparent,#000,transparent)]"
            animate={{ x: ["-110%", "210%"] }}
            transition={{ duration: 1.6, repeat: Infinity, repeatDelay: 0.5 }}
          />
        )}
        <svg viewBox="0 0 24 24" fill="currentColor" className="size-5 shrink-0" aria-hidden>
          <path d="M.057 24l1.687-6.163a11.867 11.867 0 01-1.587-5.946C.16 5.335 5.495 0 12.05 0a11.817 11.817 0 018.413 3.488 11.824 11.824 0 013.48 8.414c-.003 6.557-5.338 11.892-11.893 11.892a11.9 11.9 0 01-5.688-1.448L.057 24zm6.597-3.807c1.676.995 3.276 1.591 5.392 1.592 5.448 0 9.886-4.434 9.889-9.885.002-5.462-4.415-9.89-9.881-9.892-5.452 0-9.887 4.434-9.889 9.884a9.86 9.86 0 001.51 5.26l-.999 3.648 3.978-1.04zm11.387-5.464c-.074-.124-.272-.198-.57-.347-.297-.149-1.758-.868-2.031-.967-.272-.099-.47-.149-.669.149-.198.297-.768.967-.941 1.165-.173.198-.347.223-.644.074-.297-.149-1.255-.462-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.297-.347.446-.521.151-.172.2-.296.3-.495.099-.198.05-.372-.025-.521-.075-.148-.669-1.611-.916-2.206-.242-.579-.487-.5-.669-.51l-.57-.01c-.198 0-.52.074-.792.372s-1.04 1.016-1.04 2.479 1.065 2.876 1.213 3.074c.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.626.712.226 1.36.194 1.872.118.571-.085 1.758-.719 2.006-1.413.248-.695.248-1.29.173-1.414z" />
        </svg>
        Seguir por WhatsApp
        <ArrowRight className="size-4 transition-transform group-hover:translate-x-1" />
      </motion.a>

      <p className="mt-1.5 text-center text-[11px] text-emerald-300/90">
        ⚡ Respuesta en minutos · sin compromiso
      </p>
      <button
        onClick={onReiniciar}
        className="mx-auto mt-2 block text-[11px] text-slate-400 underline-offset-2 hover:text-slate-200 hover:underline"
      >
        Empezar de nuevo
      </button>
    </motion.div>
  );
}
