...
// hero oscuro + contador + barra de filtros + primera tanda del grid (scroll infinito).
    ...
    <>
      ...
      <div className="section-dark border-b border-white/5">
        <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
          ...
          <div className="mt-2 h-11 w-80 max-w-full animate-pulse rounded-lg bg-white/10" />
          <div className="mt-3 h-4 w-full max-w-prose animate-pulse rounded bg-white/5" />
          <div className="mt-2 h-4 w-full max-w-prose animate-pulse rounded bg-white/5" />
      ...
      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div className="mb-6 flex items-center justify-between gap-3">
          ...
          <div className="h-9 w-24 animate-pulse rounded-full bg-white/5 lg:hidden" />
        </div>

        {/* Barra de filtros (visible en lg+) */}
        <div className="hidden gap-3 rounded-2xl border border-white/10 bg-white/[0.03] p-3 lg:flex lg:flex-wrap">
          <div className="h-9 flex-1 animate-pulse rounded-lg bg-white/5 lg:min-w-56" />
          {Array.from({ length: 7 }).map((_, i) => (
            <div key={i} className="h-9 w-36 animate-pulse rounded-lg bg-white/5" />
          ))}
        </div>
